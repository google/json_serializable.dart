...
[SEVERE] json_serializable:json_serializable on lib/sample.dart:

You are missing a required dependency on json_annotation in the "dependencies" section of your pubspec with a lower bound of at least "$_annotationLowerBound".''';
...
void main() {
  ...
  test(
    ...
    () => _structurePackage(
      ...
      exitCode: 1,
      message: '''
[SEVERE] json_serializable:json_serializable on example/sample.dart:

You are missing a required dependency on json_annotation with a lower bound of at least "$_annotationLowerBound".''',
    ),
  );
...
The version constraint "any" on json_annotation allows versions before $_annotationLowerBound which is not allowed.''',
    ),
  );
...
The version constraint "^4.0.0" on json_annotation allows versions before $_annotationLowerBound which is not allowed.''',
    ),
  );
}
...
''',
  ...
  await expectLater(
    proc.stdout,
    emitsThrough(
      emitsInOrder(LineSplitter.split(message)),
    ),
  );

  await expectLater(proc.stderr, emitsDone);

  await proc.shouldExit(exitCode);
}
